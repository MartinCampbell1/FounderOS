"""Legacy shared contracts (V1) — canonical source of truth.

Both Quorum and Autopilot import from this module.  The per-plane copies
(``quorum/orchestrator/shared_contracts.py`` and
``autopilot/autopilot/core/shared_contracts.py``) are now thin re-export
shims pointing here.

These dataclass-based contracts predate ``ExecutionBriefV2`` (Pydantic).
They remain in use on the shared-brief import path and outcome export path.
"""
