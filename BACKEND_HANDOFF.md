...
- Shell repo root: `<local FounderOS checkout>`
- Web app: `<local FounderOS checkout>/apps/web`
- Quorum wrapper package: `<local FounderOS checkout>/apps/quorum-api/package.json`
- Quorum runtime root: `<local FounderOS checkout>/quorum`
- Autopilot wrapper package: `<local FounderOS checkout>/apps/autopilot-api/package.json`
- Autopilot runtime root: `<local FounderOS checkout>/autopilot`
- Managed stack launcher: `<local FounderOS checkout>/scripts/run-stack.mjs`
...
The following checks were run from `<local FounderOS checkout>`:
...
- Normal upstream JSON fetches use a default timeout of **3000 ms**
  - Source: `<local FounderOS checkout>/apps/web/lib/upstream.ts`
- Shell gateway health probes use a timeout of **1000 ms**
  - Source: `<local FounderOS checkout>/apps/web/lib/gateway.ts`
...
- Discovery sessions/detail: `<local FounderOS checkout>/apps/web/lib/discovery.ts`
- Discovery replay/session history: `<local FounderOS checkout>/apps/web/lib/discovery-history.ts`
- Discovery board data: `<local FounderOS checkout>/apps/web/lib/discovery-board.ts`
- Dashboard rollups also pull session data: `<local FounderOS checkout>/apps/web/lib/dashboard.ts`
- Parity audit also checks `orchestrate/sessions`: `<local FounderOS checkout>/apps/web/lib/shell-parity-audit.ts`
...
Current behavior in `<local FounderOS checkout>/apps/web/lib/chain-graph-data.ts`:
...
- `<local FounderOS checkout>/apps/web/lib/upstream.ts`
- `<local FounderOS checkout>/apps/web/lib/gateway.ts`
...
- `<local FounderOS checkout>/quorum/gateway.py`
- `<local FounderOS checkout>/quorum/orchestrator/api.py`
- `<local FounderOS checkout>/quorum/orchestrator/models.py`
- `<local FounderOS checkout>/quorum/orchestrator/discovery_store.py`
- `<local FounderOS checkout>/quorum/orchestrator/engine.py`

### Autopilot reference

- `<local FounderOS checkout>/autopilot/autopilot/api/serve.py`
...
```bash
cd <local FounderOS checkout>/quorum
./.venv/bin/python gateway.py
```

```bash
cd <local FounderOS checkout>/autopilot
