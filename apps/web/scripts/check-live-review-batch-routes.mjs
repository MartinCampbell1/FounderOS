...
function selectUnifiedExecutionLaneRecords(snapshot, target, lane) {
  return (snapshot.reviewCenter.json.execution?.records || []).filter((record) => {
...
function assertReviewRouteDiagnostics(diag, args) {
  ...
  assert(diag?.lane === args.lane, `Review route lane mismatch for ${args.label}.`);
  assert(
    firstString(diag?.routeScope?.projectId) === firstString(args.target.routeScope.projectId),
...
function assertDiscoveryReviewRouteDiagnostics(diag, args) {
  assert(diag?.route === "discovery-review", "Unexpected discovery review route payload.");
  assert(
...
function assertExecutionReviewRouteDiagnostics(diag, args) {
  assert(diag?.route === "execution-review", "Unexpected execution review route payload.");
  assert(
    diag?.filter === args.filter,
...
try {
  ...
  const beforeReviewLinkedRoute = await loadRouteDiagnostics(
    `/review${buildQuery({
      ...
      intake_session_id: suiteTargets["discovery-pass"].routeScope.intakeSessionId,
    })}`,
    "review"
  );
  const beforeReviewIssuesRoute = await loadRouteDiagnostics(
    `/review${buildQuery({
      ...
      intake_session_id: suiteTargets["critical-pass"].routeScope.intakeSessionId,
    })}`,
    "review"
  );
  const beforeReviewDecisionsRoute = await loadRouteDiagnostics(
    `/review${buildQuery({
      ...
      intake_session_id: suiteTargets["decision-pass"].routeScope.intakeSessionId,
    })}`,
    "review"
  );
  ...
  for (const record of [...unifiedDiscoveryRecords, ...discoveryReviewRecords]) {
  ...
  for (const record of executionIssueRecords) {
    const result = await postExecutionIssueResolve(record, "execution-review-issues");
    if (!result.skipped) {
  ...
  const afterUnifiedReviewLinkedExecutionRecords = selectUnifiedExecutionLaneRecords(
    after,
    suiteTargets["discovery-pass"],
    "linked"
  );
  ...
  for (const [ideaId, beforeDossierPayload] of discoveryRecordsByIdeaId.entries()) {
    ...
    const relatedRecords = [...unifiedDiscoveryRecords, ...discoveryReviewRecords].filter(
      ...
      const createdDecision =
        (afterDossier.decisions || []).find(
          (decision) =>
            ...
            String(decision.metadata?.source || "") === "live-review-batch-routes" &&
    ...
    JSON.stringify({
      ...
      routes: {
        review: {
          ...
          selectedDiscoveryKeys: unifiedDiscoveryRecords.map((record) => record.key),
        ...
        reviewIssues: {
          ...
          selectedIssueIds: unifiedReviewIssueRecords.map((record) => record.issue.id),
        },
        reviewDecisions: {
          lane: "decisions",
        ...
        discoveryReview: {
          ...
          scenario: suiteTargets["critical-pass"].scenario,
          routeScope: suiteTargets["critical-pass"].routeScope,
          selectedDiscoveryKeys: discoveryReviewRecords.map((record) => record.key),
        },
        executionReviewIssues: {
          ...
          scenario: suiteTargets["critical-pass"].scenario,
          routeScope: suiteTargets["critical-pass"].routeScope,
          selectedIssueIds: executionIssueRecords.map((record) => record.issue.id),
        },
