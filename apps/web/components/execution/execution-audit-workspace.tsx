...
import { ShellRecordMeta } from "@/components/shell/shell-record-primitives";
...
function ContentPreview({
  ...
  return (
    <ShellDetailCard className="bg-[color:var(--shell-control-bg)]/85" bodyClassName="mt-2.5">
      <div className="text-[12px] font-medium text-foreground">{title}</div>
      {content ? (
        <pre className="max-h-[28rem] overflow-auto whitespace-pre-wrap break-words rounded-[8px] bg-background/50 p-2.5 text-[12px] leading-6 text-muted-foreground">
          {content}
        </pre>
      ) : (
        <div className="rounded-[8px] bg-background/50 p-2.5 text-[12px] leading-6 text-muted-foreground">
          {fallback}
        </div>
      )}
    </ShellDetailCard>
...
function RelatedRunCard({
  ...
  if (!href) {
    return (
      <ShellDetailCard className="bg-[color:var(--shell-control-bg)]/85">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="text-[12px] font-medium text-foreground">{runId}</div>
  ...
  return (
    <ShellDetailLinkCard
      href={href}
      className="bg-[color:var(--shell-control-bg)]/85"
      title={runId}
      eyebrow={
        <div className="flex flex-wrap items-center gap-2">
          <Badge tone={statusTone(status)}>{status}</Badge>
      ...
      bodyClassName="mt-1.5 text-[12px] leading-6 text-muted-foreground"
    >
      created {safeFormatRelativeTime(createdAt, safeFormatDate(createdAt))}
    </ShellDetailLinkCard>
  );
}
          ...
          >
            {audit.findings.length > 0 ? (
              audit.findings.map((finding) => (
                <ShellDetailCard key={finding} className="bg-[color:var(--shell-control-bg)]/85">
