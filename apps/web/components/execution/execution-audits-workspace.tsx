...
function AuditCard({
  ...
  return (
    <ShellDetailLinkCard
      href={href}
      className="p-3"
      title={audit.summary || audit.source_name || audit.id}
      eyebrow={
        <ShellRecordMeta className="text-[11px] leading-4">
          <span>{audit.source_kind}</span>
          {audit.source_name ? <span>{audit.source_name}</span> : null}
          {audit.source_id ? <span>{audit.source_id}</span> : null}
        </ShellRecordMeta>
      }
      actions={
