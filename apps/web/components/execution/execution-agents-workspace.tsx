...
function AgentProjectCard({
  ...
  return (
    <ShellListLink href={href} className="space-y-2.5">
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {project.launch_profile?.preset ? (
          <Badge tone="neutral" className="text-[10px]">
            preset {project.launch_profile.preset}
          </Badge>
        ) : null}
        {project.provider_config?.family ? (
          <Badge tone="neutral" className="text-[10px]">
      ...
      {deliveryStatus ? (
        <ShellDetailCard className="bg-[color:var(--shell-control-bg)]/90 p-3">
          <div className="flex flex-wrap items-start justify-between gap-2.5">
            <div className="min-w-0 space-y-1">
              ...
              <div className="text-[11px] leading-4 text-muted-foreground">
                {deliveryStatus.headline}
              </div>
            </div>
            <Badge tone={statusBadgeTone(deliveryStatus.status)}>
          ...
          <div className="flex flex-wrap gap-2 text-[11px] text-muted-foreground">
            {deliveryStatus.next_step ? <span>Next: {deliveryStatus.next_step}</span> : null}
            {deliveryStatus.handoff_ref ? <span>Handoff {deliveryStatus.handoff_ref}</span> : null}
          </div>
        </ShellDetailCard>
      ) : null}
...
function RuntimeAgentCard({
  ...
  const attentionState = agent.attention?.state || "unknown";
  const content = (
    <>
      <div className="flex flex-wrap items-start justify-between gap-2.5">
        <div className="min-w-0 space-y-1">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge tone={attentionBadgeTone(attentionState)}>{attentionState}</Badge>
          {agent.pipeline_status ? (
            <Badge tone={statusBadgeTone(agent.pipeline_status)}>
              {agent.pipeline_status}
            </Badge>
          ) : null}
        </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {agent.specialist ? <Badge tone="info" className="text-[10px]">specialist</Badge> : null}
        {agent.provider ? <Badge tone="neutral" className="text-[10px]">{agent.provider}</Badge> : null}
        {agent.profile_name ? (
          <Badge tone="neutral" className="text-[10px]">
            {agent.profile_name}
          </Badge>
      ...
      {(agent.budget?.usage_label || agent.attention?.recommended_action) ? (
        <ShellDetailCard className="bg-[color:var(--shell-control-bg)]/90 p-3">
  ...
  return <ShellListLink href={href} className="space-y-2.5">{content}</ShellListLink>;
}
  ...
  const content = (
    <>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <Badge tone="neutral" className="text-[10px]">
          {run.policy_profile || "custom"}
        </Badge>
        {run.approval_required ? <Badge tone="warning" className="text-[10px]">approval required</Badge> : null}
        {run.handoff_blocked ? <Badge tone="danger" className="text-[10px]">handoff blocked</Badge> : null}
        {run.open_shadow_audit_count ? (
          <Badge tone="danger" className="text-[10px]">
            {run.open_shadow_audit_count} open audits
          </Badge>
        ) : null}
      </div>
...
export function ExecutionAgentsWorkspace({
  ...
  const projectFilterCounts: Record<AgentFilter, number> = {
    all: snapshot.projects.length,
    running: snapshot.projects.filter((project) => runtimeHealth(project) === "running")
      .length,
    attention: snapshot.projects.filter((project) => runtimeHealth(project) === "attention")
      .length,
    standby: snapshot.projects.filter((project) => runtimeHealth(project) === "standby")
      .length,
    completed: snapshot.projects.filter((project) => runtimeHealth(project) === "completed")
      .length,
  };
  ...
  return (
    <ShellPage>
      ...
      >
            ...
            {([
              {
                key: "all",
                label: "All",
                count: projectFilterCounts.all,
                icon: <Bot className="h-3.5 w-3.5" />,
              },
              {
                key: "running",
                label: "Running",
                count: projectFilterCounts.running,
                icon: <PlayCircle className="h-3.5 w-3.5" />,
              },
              {
                key: "attention",
                label: "Attention",
                count: projectFilterCounts.attention,
                icon: <AlertTriangle className="h-3.5 w-3.5" />,
              },
              {
                key: "standby",
                label: "Standby",
                count: projectFilterCounts.standby,
                icon: <CirclePause className="h-3.5 w-3.5" />,
              },
              {
                key: "completed",
                label: "Completed",
                count: projectFilterCounts.completed,
              ...
              >
                <span className="flex items-center gap-1.5">
                  {option.icon}
                  {option.label}
                  <span className="rounded-full border border-[color:var(--shell-control-border)] px-1.5 py-0.5 text-[11px] leading-none text-muted-foreground">
                    {option.count}
                  </span>
                </span>
              </ShellPillButton>
        ...
        contentClassName="grid gap-2.5 md:grid-cols-3"
      >
        <ShellDetailCard className="p-3.5">
      ...
      <ShellSectionCard
        ...
        contentClassName="grid gap-2.5 md:grid-cols-3"
      >
        <div className="rounded-[12px] border border-[color:var(--shell-control-border)] bg-[color:var(--shell-control-bg)] p-3.5">
          <div className="text-[12px] font-medium text-foreground">Tracked projects</div>
          <p className="mt-1.5 text-[11px] leading-5 text-muted-foreground">
            {snapshot.projects.length} projects are present in the shell-owned execution fleet snapshot.
          </p>
        </div>
        <div className="rounded-[12px] border border-[color:var(--shell-control-border)] bg-[color:var(--shell-control-bg)] p-3.5">
          <div className="text-[12px] font-medium text-foreground">Connected runtimes</div>
          <p className="mt-1.5 text-[11px] leading-5 text-muted-foreground">
            {connectedCount} projects currently expose live runtime control or a runtime session id.
          </p>
        </div>
        <div className="rounded-[12px] border border-[color:var(--shell-control-border)] bg-[color:var(--shell-control-bg)] p-3.5">
          <div className="text-[12px] font-medium text-foreground">Latest run activity</div>
          <p className="mt-1.5 text-[11px] leading-5 text-muted-foreground">
