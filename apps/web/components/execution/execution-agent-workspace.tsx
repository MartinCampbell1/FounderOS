  ...
  return (
    ...
    >
      {recommendations.map((recommendation, index) =>
        <ShellDetailCard key={`recommendation:${index}`} className="bg-[color:var(--shell-control-bg)]/90">
      ...
      {suggestedCommands.map((command, index) =>
        <ShellDetailCard key={`command:${index}`} className="bg-[color:var(--shell-control-bg)]/90">
          <div className="space-y-2">
            <div className="text-[14px] font-medium text-foreground">
...
export function ExecutionAgentWorkspace({
  ...
  const storyPhaseLabel = detail?.story.phase_title || detail?.story.phase_id || null;
  const projectHref =
    detail?.project_id
            ...
            >
              <ShellDetailCard className="bg-[color:var(--shell-control-bg)]/90">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {detail.current?.specialist ? <Badge tone="info">specialist</Badge> : null}
                    ...
                    {detail.current?.pipeline_status ? (
                      <Badge tone={statusTone(detail.current.pipeline_status)}>
                        {detail.current.pipeline_status}
                      </Badge>
                    ) : null}
                  </div>
              ...
              <ShellDetailCard className="bg-[color:var(--shell-control-bg)]/90">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {detail.story.status ? (
                      <Badge tone={statusTone(detail.story.status)}>{detail.story.status}</Badge>
                    ) : null}
                    {typeof detail.story.iteration === "number" ? (
                      <Badge tone="info">iteration {detail.story.iteration}</Badge>
                    ) : null}
                    {storyPhaseLabel ? <Badge tone="neutral">{storyPhaseLabel}</Badge> : null}
                  </div>
                  <div className="space-y-1 text-[12px] leading-5 text-muted-foreground">
                    <div>
