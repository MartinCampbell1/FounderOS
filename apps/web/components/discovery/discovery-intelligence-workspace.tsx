...
function IntelligenceRecordCard({
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap gap-1.5 text-[12px] text-muted-foreground">
  ...
  return (
    <ShellPage>
      <ShellHero
        ...
        description="RepoDNA and repo-graph outputs are now surfaced as a shell-native discovery route."
        meta={
          <div className="flex flex-wrap gap-1.5">
            <span className={CHIP_CLASS}>{snapshot.records.length} tracked repos</span>
            <span className={CHIP_CLASS}>{graphCoverageCount} with graph coverage</span>
            <span className={CHIP_CLASS}>{warningCount} selected warnings</span>
          </div>
        }
        actions={
          <>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <ShellMetricCard
          ...
          detail="Latest repository digests and builder priors."
        />
        <ShellMetricCard
          label="Deep Graph Coverage"
          value={String(graphCoverageCount)}
          detail="Repos with at least one repo-graph deep dive."
        />
        <ShellMetricCard
          label="Selected Warnings"
          value={String(warningCount)}
          detail="Digest and graph warnings on the active repo detail."
        />
      </div>
      ...
      <ShellSectionCard
        ...
        description="Kick off RepoDNA digest or deeper repo-graph analysis through shell-owned actions."
      ...
      {loadState !== "loading" && snapshot.records.length === 0 ? (
        <ShellEmptyState
          ...
          description="Run a Quorum repo digest or repo graph analysis first."
          className="py-10"
        />
      ) : null}
          ...
          >
            {selectedDigest ? (
              <>
                ...
                {selectedGraph ? (
                  <ShellSectionCard
                    ...
                    description="Startup territories, risk hotspots, and evidence trails synthesized by the deeper graph analysis."
                    contentClassName="space-y-3"
                  >
                    <div className="grid gap-3 xl:grid-cols-2">
                      <TokenList
              ...
              <ShellEmptyState
                ...
                description="The selected digest detail could not be recovered from Quorum."
                className="py-10"
              />
            )}
          </ShellSectionCard>
