  ...
  return (
    ...
    >
      ...
      <div className="mt-3 grid gap-2 text-[12px] text-muted-foreground md:grid-cols-2">
        ...
        <span>Updated {safeFormatRelativeTime(profile.last_updated, safeFormatDate(profile.last_updated))}</span>
  ...
  return (
    <ShellPage>
      <ShellHero
        ...
        description="Prompt profiles, reflective eval reports, and self-play pressure now live on shell-owned discovery routes, so operators can inspect prompt drift and switch active profile without leaving FounderOS."
        meta={
          <>
            <span>{snapshot.profiles.length} prompt profiles</span>
            <span>{snapshot.reflections.length} recent reflections</span>
            <span>{snapshot.matches.length} self-play matches</span>
          </>
        }
        actions={
          <>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <ShellMetricCard
          ...
          detail="Baseline, mutant, and self-improved prompt families tracked by Quorum."
        />
        <ShellMetricCard
          label="Reflections"
          value={String(snapshot.reflections.length)}
          detail="Recent reflective eval reports that explain current failure pressure."
        />
        <ShellMetricCard
      ...
      {loadState !== "loading" && snapshot.profiles.length === 0 ? (
        <ShellEmptyState
          ...
          description="Quorum has not materialized any improvement-lab profiles yet. Once they exist, this shell route will surface the active profile, reflections, and self-play history."
        />
      ) : null}
      ...
      {snapshot.profiles.length > 0 ? (
        <>
          <ShellSectionCard
            ...
            description="Active prompt profile, mutation lineage, and the reflection pressure behind the current improvement loop."
            actions={
              selectedProfile ? (
          ...
          >
            {selectedProfile ? (
              ...
              <ShellEmptyState
                ...
                description="The improvement list loaded, but the selected prompt profile could not be recovered from Quorum."
                className="py-12"
              />
            )}
          </ShellSectionCard>
          ...
          <ShellSectionCard
            ...
            description="Baseline, mutated, and self-improved profiles that can drive discovery runtime behavior."
            contentClassName="space-y-4"
          >
            <ShellHeroSearchField
              value={query}
